...
### Modification for allele specific ATACseq pipeline  12 DEC 2014
### modify strains specific SNPs into Ns 
### 			using mm9
### 			add "chr" to output fa file
###			do not check if the reference have already a SNP sites to rescue the 129-specific SNPs
my $new_total = 0;
my $already_total = 0;
my $low_confidence = 0;
...
sub create_modified_chromosome {
  ...
  foreach my $snp (@snps) {
    ...
    $lastPos = $snp->[0];
###################################do not check  already have and didn't match SNPs #############################
...
sub write_129S1_chromosome {
    ...
    open (OUT,'>',"/home/jinxu/DB/mmu9/Combin_129S1_CASTEiJ/chr_withCommonSpecific_SNPs/chr$chr.fa") or die $!;
...
sub read_snps {
    ...
    my ($chr) = @_;
    my @snps = ();
    my $file = "/home/jinxu/DB/mmu9/Combin_129S1_CASTEiJ/Specific_SNPs/chr$chr.txt.specific";
...
sub read_black6_sequence {
  ...
  my $file = "/home/jinxu/DB/mmu9/Combin_129S1_CASTEiJ/chr_withCommon_SNPs/chr$chr.fa";
